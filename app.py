...
if st.session_state.tela == "HOME":
    ...
    with st.form("f_cadastro"):
        ...
        if st.form_submit_button("✅ SALVAR"):
            ...
            st.session_state.db_os.append({
                ...
                "DESCRIÇÃO DO SERVIÇO": desc, "STATUS": stt, "VALOR_TOTAL": dias * (1870.0 if tipo == "ESCOLTA" else 970.0), "LOGO_CLI_PATH": path
            })
    ...
    if idx is not None:
        ...
        with st.form("f_edit"):
            c1, c2 = st.columns(2)
            with c1:
                e_ped = st.text_input("PEDIDO", value=d["PEDIDO"])
                e_loc = st.text_input("CLIENTE", value=d["CLIENTE"])
                e_bal = st.text_input("BALSAS", value=d["BALSAS"])
                e_h_e = st.text_input("HORA EMBARQUE", value=d["EMBARQUE"])
            with c2:
                e_stt = st.selectbox("STATUS", ["ANDAMENTO", "ENCERRADO"], index=0 if d["STATUS"]=="ANDAMENTO" else 1)
                e_h_t = st.text_input("HORA TÉRMINO", value=d["TÉRMINO"])
                e_sai = st.text_input("SAÍDA", value=d["SAÍDA"])
                e_cte = st.text_input("CTEs", value=d["CTEs"])
            e_desc = st.text_area("DESCRIÇÃO DO SERVIÇO", value=d["DESCRIÇÃO DO SERVIÇO"])
            
            if st.form_submit_button("💾 ATUALIZAR"):
                st.session_state.db_os[idx].update({
                    "PEDIDO": e_ped, "CLIENTE": e_loc, "STATUS": e_stt, "BALSAS": e_bal,
                    "EMBARQUE": e_h_e, "TÉRMINO": e_h_t, "SAÍDA": e_sai, "CTEs": e_cte, "DESCRIÇÃO DO SERVIÇO": e_desc
                })
    ...
    st.title("💰 Financeiro")
    df_f = pd.DataFrame(st.session_state.db_os) if st.session_state.db_os else pd.DataFrame()
    if not df_f.empty:
        df_enc = df_f[df_f['STATUS'] == "ENCERRADO"]
        st.metric("TOTAL ENCERRADO", f"R$ {df_enc['VALOR_TOTAL'].sum():,.2f}")
        st.table(df_enc[["O.S", "CLIENTE", "VALOR_TOTAL"]])
