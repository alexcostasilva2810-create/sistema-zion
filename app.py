...
st.set_page_config(page_title="Zion Tecnologia - Gestão Operacional", layout="wide")
...
# --- FUNÇÃO PDF (ESTILO OFICIAL TRANSDOURADA) ---
def gerar_pdf_transdourada(d):
    ...
    pdf.cell(0, 5, "GRUPO DIAS - PVH SEG / ZION TECNOLOGIA", ln=True)
    pdf.ln(10)
    ...
    pdf.ln(15)
    pdf.cell(95, 7, "__________________________", align="C")
    pdf.cell(95, 7, "__________________________", ln=True, align="C")
    pdf.cell(95, 7, f"{d.get('ASSINATURA', 'Responsável')}", align="C")
    pdf.cell(95, 7, "Fiscalização Zion", ln=True, align="C")
...
if st.session_state.pagina == "🏠 HOME":
    ...
    with st.form("form_os"):
        ...
        ini = c4.date_input("INÍCIO MISSÃO"); fim = c5.date_input("FIM MISSÃO")
        ...
        if st.form_submit_button("✅ SALVAR OPERAÇÃO", type="primary"):
            ...
            if res.status_code == 200: st.success("Salvo!"); navegar("📊 GRADE")
    ...
    if dados:
        ...
        for d in dados:
            with st.expander(f"🛠️ Ações: {d['Nº OS']} - {d['CLIENTE']}"):
                c1, c2 = st.columns(2)
                if c1.button("✏️ EDITAR", key=f"ed_{d['ID']}"): st.session_state.dados_edicao = d; navegar("📋 CADASTRO")
                pdf_b = gerar_pdf_transdourada(d); c2.download_button("📄 PDF", pdf_b, f"OS_{d['Nº OS']}.pdf", key=f"p_{d['ID']}")
    ...
    if dados:
        df = pd.DataFrame(dados)
        st.metric("O.S ATIVAS", len(df[df['STATUS'] == "Em Andamento"]))
