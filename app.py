...
st.markdown("""
    <style>
    /* Estilização dos botões e tabelas */
...
if st.session_state.tela == "HOME":
    ...
    if st.session_state.db_os:
        ...
        def gerar_pdf_financeiro(df, total):
            ...
            pdf.add_page()
            pdf.set_font("Arial", 'B', 16); pdf.cell(0, 10, "RELATÓRIO FINANCEIRO ZION", ln=True, align='C')
            pdf.set_font("Arial", size=10); pdf.cell(0, 8, f"Periodo: {data_ini.strftime('%d/%m/%Y')} a {data_fim.strftime('%d/%m/%Y')}", ln=True, align='C'); pdf.ln(5)
            pdf.set_fill_color(220, 220, 220); pdf.set_font("Arial", 'B', 9)
            pdf.cell(20, 8, "O.S", 1, 0, 'C', True); pdf.cell(50, 8, "CLIENTE", 1, 0, 'C', True); pdf.cell(30, 8, "TIPO", 1, 0, 'C', True)
            pdf.cell(30, 8, "INICIO", 1, 0, 'C', True); pdf.cell(30, 8, "FIM", 1, 0, 'C', True); pdf.cell(20, 8, "DIAS", 1, 0, 'C', True); pdf.cell(40, 8, "TOTAL", 1, 1, 'C', True)
            pdf.set_font("Arial", size=9)
            for _, r in df.iterrows():
                pdf.cell(20, 7, str(r['O.S']), 1, 0, 'C'); pdf.cell(50, 7, str(r['CLIENTE'])[:25], 1); pdf.cell(30, 7, str(r['TIPO']), 1, 0, 'C')
                pdf.cell(30, 7, str(r['INICIO']), 1, 0, 'C'); pdf.cell(30, 7, str(r['FIM']), 1, 0, 'C'); pdf.cell(20, 7, str(r['DIAS']), 1, 0, 'C')
                v = f"R$ {r['TOTAL']:,.2f}" if "ENCERRADO" in r['STATUS'] else "---"
                pdf.cell(40, 7, v, 1, 1, 'R')
            pdf.ln(5); pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, f"VALOR TOTAL: R$ {total:,.2f}", ln=True, align='R')
            return pdf.output(dest='S').encode('latin-1', 'ignore')
        ...
        st.divider(); st.metric("SOMA TOTAL (PERÍODO)", f"R$ {total_p:,.2f}")
